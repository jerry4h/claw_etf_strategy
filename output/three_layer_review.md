...
### S-P0: Walk-Forward 胜率 55.6%，样本外有效性有限  🟡 **已知限制**

**可复现值 (2026-07-25 验证)**: `compute_benchmark_relative_win_rate(config, n_windows=9)` 在当前代码+数据下输出 **55.6% (5/9)**。此前文档中的 77.8% (7/9) 来自交互式分析会话，无可复现代码支撑，经 git 溯源确认。

5 个胜利窗口共性：等权基准波动率高（策略防御层降波效果显著，Sharpe 优势来自风控而非选股）。
4 个失败窗口共性：纳指 vol 低于 step_low（防御层无降波优势）+ 评分公式黄金偏好（`score = mom - 1.1*vol` 中黄金因低波长期占据 top-2，错过中证500 上涨窗口）。

策略的 alpha 真实性由 DSR=1.0 和 MC=100% 保障，但 WF 胜率说明策略在低风险环境中可能跑输等权，这是 top_n=2 集中度 + vol 惩罚过重的结构性 trade-off。
### S-P0: 全区间参数优化无 holdout period  ✅ **已验证**

**更新 (2026-07-22)**: Train (2013-2020) Sharpe=1.605，Test (2020-2026) Sharpe=1.400。
12.8% 衰减属于轻微，年化收益反而提升（16.07% vs 14.98%）。两个时期均显著跑赢等权基准。**未检测到过拟合。**
...
**更新 (2026-07-22)**: FX hedge bug 已修复。2% 年化对冲成本下 Sharpe 仅从 1.514 降至 1.464，3% 下仍保持 1.439。FX 敞口不是关键风险。
### S-P1: 危机期相关性收敛风险
...
### S-P2: 动量窗口噪声敏感（已采用 6 周窗口缓解）

**现状**: mom_window=6 周（v3.0 末版已采纳）。6 周动量在 3-6 安全区间内，短期噪声敏感度低于 4 周。
...
| **P0** | I-2 | 无实时风控监控 | 机构 |
| **P1** | S-1 | WF 55.6%（5/9），低波环境跑输等权，已知限制 | 策略 |
| **P0** | S-2 | ~~无holdout~~ train/test已验证 | 策略 |
| **P0** | C-1 | ~~FX hedge bug~~ 已修复 | 代码 |
...
| **P1** | C-5 | ~~CI/CD~~ 已添加 | 代码 |
| **P1** | C-6 | ~~状态文件~~ 已原子写入 | 代码 |
| **P1** | C-7 | ~~测试不足~~ 80测试全通过 | 代码 |
| **P2** | I-6 | 无容量分析 | 机构 |
| **P2** | I-7 | 密钥管理不足 | 机构 |
| **P2** | S-6 | 动量窗口过短 | 策略 |
| **P2** | S-7 | 无利率敏感度分析 | 策略 |
...
6. **S-1**: ✅ 已分析 — 5/9 WF 胜率，失败窗口为低波+黄金选股拖累（见上方 S-P0 节）
