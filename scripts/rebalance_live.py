...
# 默认生产 config = v4.6 (定向 boost 分级应用, 完整对抗管线通过); main() 可用 --config 切换
# (回退前代 v4.5-pvd: config/strategy_v4_5_pvd.yaml)
_apply_cfg(load_config(PROJECT / 'config/strategy_v4_6.yaml'))
...
def main():
    ...
    p.add_argument('--config', default='config/strategy_v4_6.yaml',
                   help='策略配置(默认 v4.6 生产; 回退 v4.5-pvd 用 config/strategy_v4_5_pvd.yaml)')
    ...
    if a.config and a.config != 'config/strategy_v4_6.yaml':
