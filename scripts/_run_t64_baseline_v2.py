#!/usr/bin/env python3
"""Phase 6 Full-Grid for v2.3 baseline — using sequential execution to avoid multiprocessing issues."""
import sys, os, json, multiprocessing, numpy as np
...
from src.robustness import (
    ...
    generate_robustness_report, RobustnessResult,
)
...
print("=" * 70)
print("Phase 6 Full-Parameter Grid — v2.3 Baseline (sequential workers)")
print(f"Config: {config}")
print(f"Output: {output_dir}")
print("=" * 70)

# First get the standard metrics (without full_grid)
...
result = evaluate_robustness(
    ...
    full_grid=False,  # We'll do full grid manually
)
...
# Run grid points sequentially (avoids multiprocessing Pool pickling issues)
...
# Organize results
from src.robustness import GridPointResult
...
with open(out / 'baseline_intermediate.json', 'w') as f:
    json.dump(entry, f, indent=2, default=str, ensure_ascii=False)

print(f"\nIntermediate result saved to {out / 'baseline_intermediate.json'}")
print("Baseline Phase 6 grid complete!")