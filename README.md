...
> **军工 ETF 标的扩容评估**：评估把军工作为第 6 条腿（或替换中证500）的可行性，
> 三道先定后测门禁只过 **2/3 → NO-GO（不进 E2）**。双口径：中证军工指数 399967.SZ
> （2013 起全样本）+ 国泰中证军工ETF 512660.SH（2016-08 上市，军工 ETF 中最早且流动性最好）。
> ① **G1 低相关 FAIL** —— 与中证500 周收益相关 **0.8153**（ETF 口径 0.7783），且高波段升到
> **0.8564**、下行段 0.8359（平静段 0.7934）—— 相关性在最需要分散时反而更高；
> ② G2 可选中率 PASS（进 TOP2 占 16.19% 有效周，**推翻了“vol 惩罚使其选不中”的先验**，
> 但分年占比从 0% 到 32.1%，2023 全年零选中，属主题轮动型暴露）；
> ③ G3 独立信息 PASS 但擦边（idio share 32.31% vs 门禁 30%）—— 回归系数显示
> 军工 ≈ 中证500×1.10 − 红利低波×0.22，是**风格倾斜而非新资产类别**。
> 价格总账：年化波动 31.07% vs 25.5%、MaxDD 70.47% vs 63.48%，而相对中证500 年化超额
> **−1.65%**；下行 beta 1.004 ≥ 上行 beta 0.951。结论：扩容等于放大 A 股集中度，不是补充
> 新的风险溢价。全量表格见 `output/experiments/exp_junshi_pool.md`（脚本
> `scripts/_exp_junshi_pool_study.py`，支持 `--render-only` 幂等重算）。
